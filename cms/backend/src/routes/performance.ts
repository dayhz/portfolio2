...
router.get('/stats', (req, res) => {
  try {
    ...
    const stats = performanceMonitoringService.getStats(timeRange);
    res.json(stats);
  } catch (error) {
...
router.get('/alerts', (req, res) => {
  try {
    ...
    const alerts = performanceMonitoringService.getAlerts(limit);
    res.json(alerts);
  } catch (error) {
...
router.get('/realtime', (req, res) => {
  try {
    const realTimeStats = performanceMonitoringService.getRealTimeStats();
...
router.get('/endpoint/:endpoint', (req, res) => {
  try {
    ...
    const metrics = performanceMonitoringService.getEndpointMetrics(
...
router.delete('/cleanup', (req, res) => {
  try {
    ...
    const cutoffDate = olderThan ? new Date(olderThan as string) : new Date(Date.now() - 24 * 60 * 60 * 1000); // 24 hours ago
    
    performanceMonitoringService.cleanup(cutoffDate);
